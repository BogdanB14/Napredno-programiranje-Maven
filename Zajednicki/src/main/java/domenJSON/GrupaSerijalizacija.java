...
public class GrupaSerijalizacija implements JSONSerijalizacija {
    ...
    public void serijalizacija(ApstraktniDomenskiObjekat klasa, String putanja) {
         Gson gson = new Gson();
        String json = gson.toJson((Grupa) klasa);
        ...
        try{
            ...
            try(FileWriter writer = new FileWriter(file, postoji)){
                if(postoji)
                    writer.write("\n");
                writer.write(json);
                System.out.println("Serijalizovana grupa: " + json + " na putanji: " + putanja);
            }
        } catch(IOException e){
            System.out.println("Usao u catch u serijalizaciji grupa");
            e.printStackTrace();
        }
