...
/**
 * Klasa koja implementira metodu za serijalizaciju liste mesta.
 * @author Bogdan Blagojevic
 */
public class MestoSerijalizacija implements JSONSerijalizacija {
    ...
    public void serijalizacija(ApstraktniDomenskiObjekat klasa, String putanja) {
                  Gson gson = new Gson();
        String json = gson.toJson((Mesto) klasa);
        ...
        try{
            ...
            try(FileWriter writer = new FileWriter(file, postoji)){
                if(postoji)
                    writer.write("\n");
                writer.write(json);
                System.out.println("Serijalizovana mesta: " + json + " na putanji: " + putanja);
            }
        } catch(IOException e){
            System.out.println("Usao u catch u serijalizaciji mesta");
            e.printStackTrace();
        }
    }
